...
export default function StaffVisits() {
  ...
  // Fetch visits for current staff member
  const { data: visits, isLoading } = useQuery({
    queryKey: ['staff-visits', user?.id],
    queryFn: async () => {
      if (!user?.id) return [];
  ...
  const getCompletionBadge = (notes: string | null) => {
    ...
    if (summaryMatch) {
      ...
      if (percentage === 100) {
        return <Badge variant="default" className="bg-success">Complete ({completed}/{total})</Badge>;
      } else if (percentage >= 75) {
        return <Badge variant="default" className="bg-warning">Mostly Done ({completed}/{total})</Badge>;
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Loading your visit history...</p>
            </div>
          </div>
        </div>
        <Card>
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">My Visits</h1>
            <p className="text-muted-foreground">Your site visit history and reports</p>
          </div>
        </div>
          ...
          {visits && visits.length > 0 ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">
                You haven't logged any site visits. Get started by adding your first visit.
              </p>
              <Link to="/staff/visits/new">
                <Button>
                  <Plus className="mr-2 h-4 w-4" />
